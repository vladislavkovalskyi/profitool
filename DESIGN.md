...
Тёмная включена по умолчанию, светлую включает кнопка солнца и луны в шапке (в мобильном меню строка
внизу). Выбор хранится в `localStorage`, ключ `profitool-theme`, атрибут `data-theme="light"` на `<html>`
ставит скрипт из `<head>` до первой отрисовки, поэтому мигания нет.
